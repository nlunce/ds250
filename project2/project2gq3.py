...
filter_df['num_of_delays_late_aircraft'] = filter_df['num_of_delays_late_aircraft'].replace({-999:np.nan})
filter_df
...
filter_df['num_of_delays_late_aircraft'] = filter_df['num_of_delays_late_aircraft'].replace({np.nan: avg_delays_late_aircraft})
...
total_delays_late_aircraft = (filter_df['num_of_delays_late_aircraft'].sum() * .3)
...
sixty_five_months = ["January", "Febuary", "March", "September", "October", "November", "December"]
...
total_nas_weather = (fourty_table['num_of_delays_nas'].sum() * .4)

total_nas_weather += (sixty_five_table['num_of_delays_nas'].sum() * .65)
...
filter_df.loc[condition1, 'true_num_of_delays_weather'] = (
    ...
    (filter_df['num_of_delays_late_aircraft'] * .30) +
    (filter_df['num_of_delays_nas'] * .40)
    ).round()
...
final = filter_df.rename(columns={'num_of_delays_weather':'num_of_delays_severe_weather'} )
final = final.rename(columns={'true_num_of_delays_weather':'total_num_delays_weather'} )
final = final.filter(['month', 'num_of_delays_late_aircraft', 'num_of_delays_nas', 'num_of_delays_severe_weather', 'total_num_delays_weather' ])
final