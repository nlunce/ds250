...
month_order = ['January', 'Febuary', 'March', 'April', 'May', 'June', 'July',
...
#%%
# Graph data
graph = (alt.Chart(chart, title='Average Number of Delays by Month')
  .encode(
    x=alt.X('month:O', axis=alt.Axis(title="Month"), sort= month_order),
    y=alt.Y('average_num_of_delays:Q', axis=alt.Axis(title="Average Number of Delays"))
        ).configure_axis(
             labelFontSize=20,
             titleFontSize=30
        ).configure_title(
